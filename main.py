...
st.markdown(
    ...
    .footer {position: fixed; bottom: 0; width: 100%; background-color: #0e1117; color: white; text-align: center; padding: 10px;}
    </style>
    """,
...
with st.sidebar:
    ...
    search_query = st.text_input("Search Stocks", placeholder="Enter stock symbol (e.g., RELIANCE, TCS)")
    ...
    timeframe = st.selectbox("Timeframe", ["1d", "1h", "15m", "5m"], help="Select data timeframe (daily, hourly, or minutes)")
...
if not search_query:
    st.title("⬅️ Search for Stocks")

if search_query:
    symbol = search_query.upper()
    symbol = f"{symbol}.BO" if exchange == "BSE" else f"{symbol}.NS"
    
    if symbol not in st.session_state.watchlist and st.button("Add to Watchlist"):
        st.session_state.watchlist.append(symbol)
        st.rerun()

    # Fetch stock data
    df, stock_info = get_stock_data(symbol, start=selected_start_date, end=selected_end_date, interval=timeframe)
